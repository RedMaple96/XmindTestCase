...
# 使用新的xmind_loader替代原有的xmind库
try:
    from xmind_loader import load as xmind_load, XmindLoadError
    USE_NEW_LOADER = True
    logging.info("使用新的XMind加载器")
except ImportError:
    logging.error("无法导入xmind库，请确保已安装")
    xmind = None
...
def get_xmind_testsuites(xmind_file):
    ...
    try:
        # 使用新的xmind_loader加载文件
        loader = xmind_load(xmind_file)
        xmind_content_dict = loader.getData()
    ...
    except XmindLoadError as e:
        logging.error('XMind加载错误 - %s: %s', e.error_type, e.message)
        if e.details:
            logging.error('错误详情: %s', e.details)
        return []
    except FileNotFoundError as e:
        logging.error('XMind文件不存在: %s', str(e))
        return []
    except PermissionError as e:
        logging.error('XMind文件访问权限错误: %s', str(e))
        return []
    except Exception as e:
        logging.error('Failed to load XMind file(%s): %s', xmind_file, str(e))
        return []
